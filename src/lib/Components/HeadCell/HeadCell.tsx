...
const HeadCell: React.FunctionComponent<IHeadCellProps> = (props) => {
  ...
  const { elementAttributes, content } = getElementCustomization({
    className: `${defaultOptions.css.theadCell} ${defaultOptions.css.theadCellHeight} ${defaultOptions.css.theadBackground} ${isSortingEnabled(sortingMode) ? 'ka-pointer' : ''} ${isGrouped ? 'ka-thead-grouped-cell' : ''}`,
    colSpan,
    rowSpan,
    scope: 'col',
    style,
